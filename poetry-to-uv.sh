...
full_path=$(cd "$(dirname "$input_file")"; pwd)/$(basename "$input_file")  # Updated line
dir_path=$(dirname "$full_path")
output_file="${dir_path}/pyproject.toml"
backup_file="${dir_path}/pyproject.toml.poetry_backup"
...
clean_dependency() {
    if echo "$1" | grep -q 'python\s*=\s*"'; then
        # If the line contains 'python ="', return an empty string to effectively remove the line
        echo "" 
    elif echo "$1" | grep -q 'path\s*=\s*'; then
        echo "$1" | sed -e 's/^.*\s=\s{/# {/g'
    elif echo "$1" | grep -q 'extras'; then
        echo "$1" | sed -e 's/\s=\s//g' \
            -e 's/{.*extras.*\(\[.*\]\).*version"^\(.*\)}/\1>=\2/' \
            -e 's/\"//g' \
            -e 's/^/"/' # Add quote at start
    else
